...
export default class Hentai extends Module{
    ...
    */
    ...
    public async Run(interaction: Discord.ChatInputCommandInteraction){
        ...
        if(!channel) throw new SynergyUserError("You can't use this command here.");
            ...
            case Discord.ChannelType.DM: {
                ...
                if(!access){
                    if(!access) throw new SynergyUserError("To use this command in DM you need to confirm your age.", 
                                                            "To confirm that you're 18+ years old run `/config set namespace: user field: allow_nsfw value_bool: true` command.");
                }
                break;
            }
            default: {
