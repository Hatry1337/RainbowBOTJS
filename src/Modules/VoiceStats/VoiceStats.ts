...
export default class VoiceStats extends Module{
    ...
    private async onRVoiceChannelJoin(channel: Discord.VoiceChannel, member: Discord.GuildMember){
        ...
        if(!session){
            ...
            this.Logger.Info(`[JoinEvent] Wtf why session exists on join?!! Debug this shit pls... trace id is: ${GlobalLogger.Trace({channel, member, session})}`);
        }
    }
    ...
    public async Init(){
        ...
        this.Logger.Info(`[Init] Created ${loadc} sessions for users currently in channels.`);
    ...
    public async Run(interaction: Discord.CommandInteraction, user: User){
        ...
        for(let e of mtimetotal){
            stat += `${i}. <@${e[0]}> - ${Utils.formatTime(e[1])} in voice channels\n`;
            if(i >= 10){
                break;
            }
            i++;
