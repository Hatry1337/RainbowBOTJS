...
class Buy {
    ...
    execute = function (message) {
        ...
        this.Database.getUserByDiscordID(message.author.id, function (user) {
            var bs1_pr = 50000 * 1.2^(user.bitminer1-1);
            var bs2_pr = 100000 * 1.2^(user.bitminer1-1);
            var br_pr  = 200000 * 1.2^(user.bitminer1-1);
            var brd_pr = 1200000 * 1.2^(user.bitminer1-1);
            var ss_pr  = 12000000 * 1.2^(user.bitminer1-1);
            ...
            if (user.user_points < (count * getPrice(parseInt(args[1])))) {
                ...
                if (args[1] === "6") {
                    ...
                    othis.Database.updateUser(user.discord_id, user, function () {
                        ...
                        othis.Database.writeLog('Buy', message.author.id, `{"Message":"User '${message.author.id}' buyed ${getProd(parseInt(args[1]))} ${count} pcs."}`);
                        if (user.bm1_time === 0) {
